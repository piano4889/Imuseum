...
<head>
<meta charset="UTF-8">
<title>IMUSEUM - YearLearn</title>
<link rel="stylesheet" type="text/css" href="/css/mystyle.jspf">
...
<body>
	...
	<div id="layout2">
		<div class="container2 notosanskr">
			<div class="box2">
				<c:forEach var="yearLearn" step="1000" items="${yearLearn }">
					<p class="now">${yearLearn.regdate2.substring(0,4) }</p>
				</c:forEach>
				<ul class="new" style="padding-top:3rem;">
					<span style="margin-right: 1100px; color: #c3c4c7;">전체 <b style="color: black">${yearLearnTotal }</b>건</span>
				...
				<c:choose>
						<c:when test="${empty yearLearn }"> 
							<h2 style="margin: auto; text-align: center; height: 400px; padding-top:10rem; border-bottom: 1px solid black;">지금은 준비 중입니다.<br>더 나은 프로그램으로 찾아뵙겠습니다.</h2>
						</c:when>
					<c:otherwise>
					<c:forEach var="yearLearn" items="${yearLearn }">
						<div id="click_area" class="click_area"><a href="getLearn.do?learnIdx=${yearLearn.learnIdx }">
							<div class="left">
								<ul>
									<li><img src="${pageContext.request.contextPath}/learnImage/${yearLearn.fileName }" width="630px" height="400px" /></li>
								</ul>
							</div>
							<div class="right">
								<div style="display: black; margin-top: 30px; margin-bottom: 20px;">
									<c:if test="${yearLearn.educationNum == 0 }">
									...
									<c:if test="${yearLearn.educationNum == 1 }">
									...
									<c:if test="${yearLearn.educationNum == 2 }">
									...
									<c:if test="${yearLearn.placeNum == 0 }">
									...
									<c:if test="${yearLearn.placeNum == 1 }">
								...
								<span style="display:block; margin-top: 10px; font-size: 30px; font-weight: bold;">${yearLearn.title }</span><br>
								<span style="font-size: 16px; color: #c3c4c7;">${yearLearn.subtitle }</span>
								<table style="width: 600px; ">
									<tr>
										<td>교육 년도</td>
										<td>${yearLearn.regdate2.substring(0, 4) }</td>
									</tr>
									<tr>
										<td>장소</td>
										<td>${yearLearn.place }</td>
									</tr>
									<tr>
										<td>정원</td>
										<td>${yearLearn.memberNumber }</td>
									</tr>
									<tr>
										<td>교육시간</td>
										<td>${yearLearn.educationTimeNum }, ${yearLearn.educationTime }</td>
									</tr>
									<tr>
										<td>요금</td>
										<td>${yearLearn.fee }</td>
