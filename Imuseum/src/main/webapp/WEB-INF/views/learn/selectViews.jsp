...
<head>
<meta charset="UTF-8">
<title>IMUSEUM - selectViews</title>
...
<body>
	...
	<div id="layout2">
		<div class="container2 notosanskr">
			<div class="box2">
				<c:forEach var="selectViews" step="1000" items="${selectViews }">
					<p class="now" style="font-weight: bold;">조회순</p>
				</c:forEach>
				<ul class="new">
					<span style="margin-right: 1100px; color: #c3c4c7;">전체 <b style="color: black">${selectviewsTotal }</b>건</span>
				...
				<c:choose>
					...
					<c:otherwise>
					<c:forEach var="selectViews" items="${selectViews }">
						<div id="click_area" class="click_area"><a href="getLearn.do?learnIdx=${selectViews.learnIdx }">
							<div class="left">
								<ul>
									<li><img src="${pageContext.request.contextPath}/learnImage/${selectViews.fileName }" width="630px" height="400px" /></li>
								</ul>
							</div>
							<div class="right">
								<div style="display: black; margin-top: 30px; margin-bottom: 20px;">
									<c:if test="${selectViews.educationNum == 0 }">
									...
									<c:if test="${selectViews.educationNum == 1 }">
									...
									<c:if test="${selectViews.educationNum == 2 }">
									...
									<c:if test="${selectViews.placeNum == 0 }">
									...
									<c:if test="${selectViews.placeNum == 1 }">
								...
								<span style="display:block; margin-top: 10px; font-size: 30px; font-weight: bold;">${selectViews.title }</span><br>
								<span style="font-size: 16px; color: #c3c4c7;">${selectViews.subtitle }</span>
								<table style="width: 600px; ">
								
									<tr>
										<td>교육 날짜</td>
										<td>${selectViews.regdate1.replaceAll("/","-") } ~ ${selectViews.regdate2.replaceAll("/","-") }</td>
									</tr>
									<tr>
										<td>장소</td>
										<td>${selectViews.place }</td>
									</tr>
									<tr>
										<td>정원</td>
										<td>${selectViews.memberNumber }</td>
									</tr>
									<tr>
										<td>교육시간</td>
										<td>${selectViews.educationTimeNum }, ${selectViews.educationTime }</td>
									</tr>
									<tr>
										<td>요금</td>
										<td>${selectViews.fee }</td>
									</tr>
									<tr>
										<td>조회수</td>
										<td>${selectViews.views }</td>
									</tr>
								</table>
							</div>
						</a></div>
