		...
		<div class="container3 notosanskr">
			<div class="box3">
				...
				<table style="width: 800px; ">
					<tr>
						<td><b>교육 진행</b></td>
						<td colspan="3">${learn.regdate1.replaceAll("/","-") } ~ ${learn.regdate2.replaceAll("/","-") }</td>
						<td><b>교육 시간</b></td>
	...
	<div id="layout4">
		<div class="container4 notosanskr">
			<div class="box4">
				...
				<c:if test="${not empty learn.fileName2 }">
					<img style="width: 49%;" src="${pageContext.request.contextPath}/learnImage/${learn.fileName2 }"/>
				</c:if>
			</div>
		</div>
	</div>
		...
		<div class="container5 notosanskr">
			...
			<c:if test="${not empty member.id }">	
				<div id="heartArea">
					<c:choose>
						<c:when test="${count == 0 }">
							<form action="updateLike.do?learnIdx=${learn.learnIdx }&id=${member.id }" method="post">
								<button type="submit" class="btn btn-warning" id="like_btn" style="float: left; padding: 15px 40px; margin-right: 5px; box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2); ">추천 <b><i>${learn.likeHit }</i></b></button>
							</form>	
						</c:when>
						<c:otherwise>
							<button type="submit" class="btn btn-warning" id="like_btn2" style=" float: left; padding: 15px 40px; margin-right: 5px; box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);">추천 완료 <b><i>${learn.likeHit }</i></b></button>
							<form action="updateLikeCancel.do?learnIdx=${learn.learnIdx }&id=${member.id }" method="post">
								<button type="submit" class="btn btn-danger" id="hate_btn" style="padding: 15px 40px; margin-right: 5px; box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);">비추천</button>
							</form>
						</c:otherwise>
					</c:choose>
