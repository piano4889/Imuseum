...
<body>
			...
			<div class="col-8">
				...
				<div class="row">
					...
					<div class="container-fluid mt-5">
						...
						<div class="pt-2">
						  <ul class="row p-0" style="list-style:none;">
						  	<li class="col-6 m-0 p-0">
						  		<a>
							  		<button onclick="location.href='mp_reviewWrite.do'" type="button" style="width:100%;" class="bt py-2 m-0">작성가능한 리뷰</button>
						  		</a>
					  		</li>
						  	<li class="col-6 m-0 p-0">
						  		<a>
							  		<button onclick="location.href='mp_review.do'" type="button" style="width:100%; color:white; background-color:black;" class="bt py-2 m-0">작성한 리뷰</button>
						  		</a>
					  		</li>
						  </ul>
						</div>
						...
						<table class="rvtable" style="width: 100%; border-top:2px solid black; margin-bottom:100px;">
							  <tr>
							  	<c:choose>
									<c:when test="${empty productReview }">
										<tr>
											<td colspan="2" style="padding:25px; text-align: center;">작성한 리뷰가 없습니다.</td>
										</tr>
									</c:when>
								<c:otherwise>
									<c:forEach var="productReview" items="${productReview }">
										<tr style="width:100%; text-align: center; height: 50px; background-color: #EBECF0;">
										    <th class="align-middle" style="float:left; margin:10px;">${productReview.productName }</th>
											<th class="align-middle" style="float:right; margin:10px; color:#E3242B;">
												<c:if test="${productReview.star == 1}">
												...
												<c:if test="${productReview.star == 2}">
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star"></i>
													<i class="bi bi-star"></i>
												...
												<c:if test="${productReview.star == 3}">
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star"></i>
												...
												<c:if test="${productReview.star == 4}">
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
												...
												<c:if test="${productReview.star == 5}">
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
													<i class="bi bi-star-fill"></i>
										...
										<tr style="width:100%; text-align:center; height: 50px; border-bottom: 1.5px solid black;">
											<td style="float:left; margin:10px;">${productReview.content }</td>
											<td style="float:right; margin:10px;">
												<input type="button" class="whitebtn" value="수정" id="writeModal" data-toggle="modal" data-target="#modal-review" 
															data-id="${productReview.prNum}^?${productReview.productName}^?${productReview.star}^?${productReview.content}">
												<input type="button" class="whitebtn" value="삭제" onclick="location.href='productReviewDelete.do?prNum=${productReview.prNum }'">
											</td>
										</tr>
									</c:forEach>
								</c:otherwise>
						...
						<div class="modal-dialog modal-dialog-centered modal-lg" >
							<div class="modal-content">
								<div class="modal-header">
									<span style="font-size: 22px; font-weight: bold;">리뷰 수정하기</span>
									<button type="button" class="close" data-dismiss="modal">
										<span aria-hidden="true" style="color: white;">&times;</span>
									</button>
								</div>
								<form action="productReviewUpdate.do" class="reviewModal">
									<div class="modal-body">
										<div class="form-group" id="form-group">
											...
											<div class="form-group row">
												...
												<div style="display: inline;margin-top:18px; font-weight: bold">별점을 입력해주세요.</div>
											</div>
										<label for="content" class="" style="font-weight: bold">리뷰 내용</label>	
									...
									<div class="modal-footer d-flex justify-content-center">
										...
										<input type="hidden" name="prNum" id="md-prNum" value="0">
									</div>
								</form>
							</div>
						</div>
 ...
 $(document).on("click", "#writeModal", function () {
	    ...
	    $('input[name=prNum]').val(infos[0]); //prNum
	    $('#md-productName').html(infos[1]); //productName
	    //$('input[name=star]').val(infos[2]); //star
	    $('textarea[name=content]').val(infos[3]); //content
})
