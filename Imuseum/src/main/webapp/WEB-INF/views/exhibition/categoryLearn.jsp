...
<%@page import="com.spring.Imuseum.learn.LearnPagingVO"%>
			...
			<div class="box1">
				<c:forEach var="categoryLearn" step="1000" items="${categoryLearn }">
					<c:if test="${not empty categoryLearn.categoryNum }">
						<p class="now">${categoryLearn.categoryName }</p>
					</c:if>
				</c:forEach>
				<ul>
						...
						<div id="click_area" class="click_area"><a href="getLearn.do?learnIdx=${categoryLearn.learnIdx }">
							...
							<div class="right">
								...
								<table style="width: 600px; ">
									...
									<tr>
										<td>교육 날짜</td>
										<td>${categoryLearn.regdate1 } ~ ${categoryLearn.regdate2 }</td>
									</tr>
									<tr>
										<td>장소</td>
					...
					<c:forEach var="categoryLearn2" items="${categoryLearn2 }">
						<ul style="border: 1px solid black; width: 400px; margin-top: 50px; margin-left: 50px; padding: 1px;" id="beforeProgram">
							...
							<li style="font-weight: bold; font-size: 22px; margin-top: 10px; padding: 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; word-break:break-all">${categoryLearn2.title }</li>
		                     ...
		                     <li style="margin-top: 50px; margin-bottom: 10px; padding: 20px;"><i>${categoryLearn2.regdate1 } ~ ${categoryLearn2.regdate2 }</i>
		                     <c:if test="${member.id == 'admin' }">
		                        <input type="button" value="X" style="float: right; border: none; background-color: white; font-weight: bold; font-size: 15px;" id="learnDelete2" onclick="javascript:location.href='learnDelete2.do?learnIdx=${categoryLearn2.learnIdx}'">
		                     </c:if></li>
		                  </ul>
					</c:forEach>
					</c:otherwise>
